...
import com.eelection.SavedLocationModel;
...
public class GetUsersLocationActivity extends AppCompatActivity implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, com.google.android.gms.location.LocationListener {
    ...
    List<Pair<String, String>> adds = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        add.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(GetUsersLocationActivity.this, AddAddressActivity.class);
        ...
        search.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence st, int start, int before, int count) {
                ...
                if (newText.length() > 1) {
                    ...
                    ref = FirebaseDatabase.getInstance().getReference().child("Addresses");
    ...
    public void displayrecentsearch(){
        ...
        int serachnumber=pref.getInt("addressnumber", 0);

        for(int i=serachnumber;i>=1;i--){
            String srchkey="recentadd"+Integer.toString(i);
            String itm=pref.getString(srchkey, null);

            String srchkeyid="recentaddid"+Integer.toString(i);
    ...
    public void displaysavedaddress(){
        ...
        for(int i=serachnumber;i>=1;i--){
            ...
            srchkey="addname"+Integer.toString(i);
            itm=pref.getString(srchkey, null);
            if(itm!=null)
                model.setInstname(itm);

            srchkey="addid"+Integer.toString(i);
            itm=pref.getString(srchkey, null);
            if(itm!=null)
                model.setInstid(itm);
    ...
    public void onLocationChanged(Location location) {
        ...
        try {
            ...
            if (addresses.isEmpty()) {
                ...
                String add=returnedAddress.getPostalCode(),found="0";

                for(int i=0;i<adds.size();i++){
                    if(adds.get(i).second.contains(add)){

                        SharedPreferences pref = getApplicationContext().getSharedPreferences("MyPref", MODE_PRIVATE);
                        @SuppressLint("CommitPrefEdits") SharedPreferences.Editor searchhint = pref.edit();

                        searchhint.putString("useraddress",adds.get(i).first);
                        searchhint.putString("useraddressid",adds.get(i).second);

                        found="1";

                        searchhint.apply();

                        Intent intent = new Intent(GetUsersLocationActivity.this, BottomNaviagtionDrawer.class);
                        startActivity(intent);
                        finish();
                    }
                }
    ...
    public void addSet(){
        ...
        ref.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for(DataSnapshot dataSnapshot1 :dataSnapshot.getChildren()){
                    adds.add(new Pair(dataSnapshot1.child("name").getValue(String.class),dataSnapshot1.child("id").getValue(String.class)));
                }
            }
            @Override
            public void onCancelled(@NonNull DatabaseError databaseError) {
